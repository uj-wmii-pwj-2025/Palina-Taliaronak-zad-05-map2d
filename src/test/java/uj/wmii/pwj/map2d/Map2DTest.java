...
class Map2DTest {
    ...
    void putThrowsNPE() {
        ...
        try {
            ...
            //left blank intentionally, exception is expected
        }
        try {
    ...
    @Test
    void containsValueRowColumnKey() {
        Map2D<String, Integer, Double> map = Map2D.createInstance();
        String rowKeyA = "A";
        int colKey1 = 1;
        String rowKeyB = "B";
        ...
        assertFalse(map.containsValue(valA1));
        assertFalse(map.containsKey(rowKeyA, colKey1));
        assertFalse(map.containsRow(rowKeyA));
        assertFalse(map.containsColumn(colKey1));
        assertFalse(map.containsKey(rowKeyB, colKey2));
        assertFalse(map.containsRow(rowKeyB));
        assertFalse(map.containsColumn(colKey2));
        assertFalse(map.containsKey(rowKeyA, colKey2));
        assertFalse(map.containsKey(rowKeyB, colKey1));

        map.put(rowKeyA, colKey1, valA1);

        assertTrue(map.containsValue(valA1));
        assertTrue(map.containsKey(rowKeyA, colKey1));
        assertTrue(map.containsRow(rowKeyA));
        assertTrue(map.containsColumn(colKey1));
        assertFalse(map.containsKey(rowKeyB, colKey2));
        assertFalse(map.containsRow(rowKeyB));
        assertFalse(map.containsColumn(colKey2));
        assertFalse(map.containsKey(rowKeyA, colKey2));
    ...
    void rowMapView() {
        ...
        //TODO check if view is unmodifiable
    }
    ...
    void copyWithConversion() {
        Map2D<String, Integer, Double> map = create2x2map();
        Map2D<String, Long, String> target = map.copyWithConversion(String::toLowerCase, Map2DTest::multiply, Object::toString);

        assertEquals(4, target.size());
        assertTrue(target.containsRow("a"));
        assertTrue(target.containsRow("b"));
        assertFalse(target.containsRow("A"));
        assertFalse(target.containsRow("B"));
        assertTrue(target.containsColumn(1L));
        assertTrue(target.containsColumn(4L));
        assertFalse(target.containsColumn(2L));
    }
    ...
    void copyWithConversionMerge() {
        ...
        assertTrue(target.containsRow("x"));
        assertFalse(target.containsRow("A"));
        assertFalse(target.containsRow("B"));
        assertTrue(target.containsColumn(1));
        assertFalse(target.containsColumn(2));
    }
